...
for j, L in enumerate(Lvalues):
    ...
    for i, T in enumerate(T_s):
        ...
        for k in range(2*Echantillon-1):
        ...
        nb_pas[i]=len(m_T)-2*Echantillon
        while (len(m_T)<2000):
            Reseau = MonteCarlo(n, L, Reseau, T,J,h)
            m_T=np.append(m_T,np.abs(np.mean(Reseau)))
    ...
    plt.savefig(f'V1-1_Configuration finale-critère Estimation par intervalle_CI Aléatoire_L={L}.pdf', bbox_inches='tight')
    plt.show()
    ...
    plt.savefig(f'V1-1_Evolution-Estimation par intervalle_CI Aléatoire_L={L}.pdf', format="pdf",bbox_inches='tight')
    plt.show()
