...
for j, N in enumerate(Nvalues):
    ...
    for i, T in enumerate(T_s):
        ...
        while (p<0.95):
            for k in range(50):
                ...
                m_T=np.append(m_T,np.abs(np.mean(Reseau)))
            p=0
            µ=np.mean(m_T[len(m_T)-50:])
            σ=np.std(m_T[len(m_T)-50:])
            for k in range(50):
                if µ-2*σ<=m_T[len(m_T)-(51+k)]<=µ+2*σ:
                    p+=1
            p/=50
    ...
    plt.savefig(f'ising_aimantation stabilisée-critère Estimation par intervalle-N={N}.pdf', bbox_inches='tight')
    plt.show()
    ...
    plt.title(f'Evolution de l\'aimantation sur critère Estimation par intervalle,N={N}')
    plt.savefig(f'Evolution de l\'aimentation-critère Estimation par intervalle_N={N}.pdf', format="pdf",bbox_inches='tight')
    plt.show()
